...
# --- #42: `calibrate diff` — the rubric-iteration loop -----------------------
#
# The judge is the deterministic containment backend, so a rubric revision has a
# predictable effect: drop the Correctness criterion and the judge stops checking
# whether the expected words appear, which is the only check a correct paraphrase
# fails. That is a rubric change with a real, hand-countable consequence, which
# is what these tests need — the statistics have their own known-answer tests in
# tests/test_delta.py.
