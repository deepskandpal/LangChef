...
<ul>
  ...
  <li><strong>Retrieval is compared on continuous scores, not thresholded.</strong> recall@k, MRR
  and nDCG go through a paired comparison of their own: Wilcoxon signed-rank with a bootstrap
  interval over the per-example differences, because those scores are bounded, pile up at the ends,
  and are nowhere near normal at the sizes anybody labels. Reducing them to pass or fail was
  considered and rejected: recall 0.42 and 0.71 are both "fail" against a threshold of 0.8, and the
  gap between them is the entire finding. See DECISIONS #12.</li>
  <li><strong>Classification needs no reduction either.</strong> Its per-example outcome is
  <code>predicted == ideal</code>, which is binary natively, so the paired test above applies
  unchanged and nothing is lost. Multi-class kappa is a non-goal: it measures agreement between two
  raters, and a hard label has no second rater.</li>
  <li><strong>No graded relevance.</strong> nDCG here is over binary relevance. A document is
  relevant or it is not. Graded judgements are a real thing and this does not do them.</li>
...
<div class="callout warn">
  ...
  <p>So on those three classes the calibration commands <strong>refuse at exit 2</strong> rather
  than returning a number nobody can interpret. The refusal names the class, says why in the same
  sentence, and lists what does apply.</p>
</div>

<div class="scroller"><table>
<thead><tr><th>Command</th><th>Free-text target<br><code>qna</code>, <code>generation</code></th><th>Hard target<br><code>classification</code>, <code>retrieval</code>, <code>reranking</code></th></tr></thead>
<tbody>
<tr><td><code>label plan</code></td><td>yes</td><td><strong>exit 2</strong></td></tr>
<tr><td><code>calibrate report</code></td><td>yes</td><td><strong>exit 2</strong></td></tr>
<tr><td><code>calibrate diff</code></td><td>yes</td><td><strong>exit 2</strong></td></tr>
<tr><td>the disagreement taxonomy</td><td>yes, inside <code>calibrate report</code> and <code>calibrate diff</code></td><td><strong>exit 2</strong>, with them</td></tr>
<tr><td><code>compare</code></td><td>yes</td><td><strong>yes</strong></td></tr>
<tr><td><code>experiment design</code> / <code>approve</code> / <code>readout</code></td><td>yes</td><td><strong>yes</strong></td></tr>
</tbody></table></div>

<p>The taxonomy has no command of its own. It groups disagreements by the criterion the judge cited,
so it exists only where a judge does, and it is reported inside the two <code>calibrate</code>
commands that refuse with it.</p>
