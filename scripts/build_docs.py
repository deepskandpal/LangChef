...
<ul>
  ...
  <li><strong>The <code>feasible</code> flag is not "can I run this".</strong> It means the required
  n is under 100,000, so a design needing 628 goldens when you have 90 still reports
  <code>feasible: true</code>. The caveats carry the real constraint. If you are reading the JSON
  rather than the prose, read <code>caveats</code> and compare <code>required_n</code> against
  <code>n_available</code> yourself.</li>
</ul>
